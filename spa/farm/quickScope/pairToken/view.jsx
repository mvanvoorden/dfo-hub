var PairToken = React.createClass({
    ...
    renderCalculation(calculation) {
        ...
        return (<section>
            {calculation.path.map((it, i) => [<span>
                {window.fromDecimals(calculation.amounts[i], it.decimals)} {it.symbol} <img width="50" src={it.logo} />
            </span>,
            i === calculation.path.length -1 ? undefined : ">"
        ])}
        </section>);
    },
    componentDidMount() {
    ...
    render() {
        ...
        return (<section>
                ...
                <section>
                    <img width="50" src={this.props.pair[selection].logo} />
                    <span>{this.props.pair[selection].name}</span>
                    <span>({this.props.pair[selection].symbol})</span>
                </section>
                <section>
                    <img width="50" src={this.props.pair[otherSelection].logo} />
                ...
                {this.state && this.state.calculation && <section>
                    {this.renderCalculation(this.state.calculation)}
                </section>}
        </section>);
    }
});